...
const DetailTouristSpot = () => {
    ...
    const handleBookingSubmit = async (e) => {
        ...
        if (response.status == 201) {
            ...
            setBookingStatus({ loading: false, error: response.message || 'Terjadi kesalahan saat membuat booking.', success: false });
        }
    };
                    ...
                    <div className="lg:col-span-3">
                        ...
                        <div className="mt-4 grid grid-cols-5 gap-2">
                            {spot.imageUrls.map((url, index) => (
                                <button key={index} onClick={() => setSelectedImageIndex(index)} className={`aspect-w-1 aspect-h-1 rounded-md overflow-hidden focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${selectedImageIndex === index ? 'ring-2 ring-blue-500' : ''}`}><img src={url} alt={`Thumbnail ${index + 1}`} className="w-full h-full object-cover" /></button>
                            ))}
                        </div>
                    </div>
                    <div className="lg:col-span-2">
                        <h1 className="text-4xl font-extrabold text-blue-900 tracking-tight">{spot.name}</h1>
                        <div className="flex items-center text-gray-600 mt-3"><FaMapMarkerAlt className="mr-2 text-blue-500" /><span className="text-lg">{spot.location}</span></div>
                        ...
                        <div className="mt-8 p-6 bg-white rounded-lg shadow-md border border-gray-200">
                            <div className="flex items-center">
                                ...
                                <div><p className="text-sm text-gray-500">Harga per Orang</p><p className="text-2xl font-bold text-blue-800">{formatPrice(spot.pricePerPerson)}</p></div>
                            </div>
                            <div className="mt-6">
                                <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2"><BsFillCalendarDateFill className="text-blue-600" />Tanggal Tersedia</h3>
                                <ul className="mt-3 space-y-2">
                                    {spot.availableDates.map(date => (<li key={date} className="text-gray-700 bg-blue-50 p-2 rounded-md">{formatIndonesianTimestampWithDateFns(date)}</li>))}
                                </ul>
                            </div>
                        </div>
                        <div className="mt-8">
                ...
                >
                    ...
                    >
                        <div className="flex justify-between items-center border-b pb-4 mb-6">
                            <h2 className="text-2xl font-bold text-blue-900">Formulir Booking</h2>
                        ...
                        <form onSubmit={handleBookingSubmit}>
                            <div className="space-y-6">
                                <div>
                                    <label htmlFor="bookingDate" className="block text-sm font-medium text-gray-700 mb-2">Pilih Tanggal</label>
                                    <select
                                        id="bookingDate"
                                        value={bookingDate}
                                ...
                                <div>
                                    <label htmlFor="numberOfPeople" className="block text-sm font-medium text-gray-700 mb-2">Jumlah Orang</label>
                                    <input
                                        type="number"
                                        id="numberOfPeople"
                                ...
                                <div className="p-4 bg-blue-50 rounded-lg text-center">
                                    ...
                                    <p className="text-3xl font-bold text-blue-900">{formatPrice(totalPrice)}</p>
                                </div>
                                {bookingStatus.error && (
